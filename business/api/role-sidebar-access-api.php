...
if (!is_business_admin($conn)) {
    flash('error', 'Only Admin can update role permissions.');
    redirect('../role-permissions.php');
}
...
try {
    ...
    log_activity($conn, 'Role Permissions', 'update', $roleId, null, $menuIds);
    mysqli_commit($conn);

    flash('success', 'Role permissions updated successfully.');
} catch (Throwable $e) {
    mysqli_rollback($conn);
    flash('error', 'Role permission update failed: ' . $e->getMessage());
}

redirect('../role-permissions.php?role_id=' . $roleId);
?>
