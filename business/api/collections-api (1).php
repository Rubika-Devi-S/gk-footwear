...
function cc_pending_filters(mysqli $conn, $businessId, $allowedIds, &$sql, &$types, array &$params) {
    ...
    $sql .= " WHERE b.business_id = ?
                ...
                AND (b.balance_amount > 0 OR b.payment_status IN ('pending','partial'))
                AND NOT (b.payment_status = 'paid' AND b.balance_amount <= 0)";
    $types .= 'i';
    $params[] = $businessId;
    ...
    try {
        $sql = "
            ...
            WHERE business_id = ?
              ...
              AND (balance_amount > 0 OR payment_status IN ('pending','partial'))
              AND NOT (payment_status = 'paid' AND balance_amount <= 0)
        ";
        $types = 'ii';
        $params = array($businessId, $billId);
        if (is_array($allowedIds)) {
